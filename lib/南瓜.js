...
var rule = {
	...
	搜索:`js:
        ...
        html = JSON.parse(html);
        var keyword = input.match(/video_name=(.*?)&/)?.[1] || "";
        html.data.forEach(function(it) {
        	// if (typeof(it.video_name) === "string" && it.video_name.indexOf(keyword) === -1){
        	// 	return;
			// }
			if (typeof(it.video_name) === "string" && keyword != "" && !it.video_name.match(new RegExp(keyword,"i"))){
        		return;
			}
            d.push({
                title: it.video_name,
