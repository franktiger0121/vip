var rule = {
    ...
    // 模拟获取的链接
    let link = "https://codeberg.org/vip3/vip/raw/branch/master/lib/urls1.txt";

    // 发起请求并解析返回的文本数据
    let textResponse = request(link);

    try {
        // 假设返回的文本数据是 JSON 格式的内容
        ...
        if (url) {
            // 使用提取的 URL 进行新的请求
            let newResponse = JSON.parse(request(url));
